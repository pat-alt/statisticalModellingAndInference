...
GA <- matrix(rnorm(n*p), nrow = n, ncol = p) %*% sqrt(t(Sigma))
# Gaussian mixture:
gaus_mix <- list(
  gaus_1 = matrix(rnorm(n*p), nrow = n, ncol = p) %*% sqrt(t(Sigma)),
  gaus_2 = matrix(rnorm(n*p), nrow = n, ncol = p) %*% sqrt((25 * t(Sigma)))
)
...
LN <- exp(GA)
# T-distribution:
T1 <- matrix(rt(n*p,1), nrow = n, ncol = p) %*% sqrt(t(Sigma))
# Truncated T:
TT <- T1
TT[TT>p] <- p
TT[TT<(-p)] <- -p
...
\end{equation}
$$
Hence the numerical exercise in this section is very similar to the one above and can be summarized as follows:
...
A number of interesting data sets related to COVID-19 have been made publicly available on Kaggle. 
              ...
              lapply( # Loop of n folds:
                ...
                function(k) {
                  ...
                  if (weighted & method_name!="UNIF") {
                    fitted_model <- glm(y_train ~ X_train, family = "binomial", weights = weights)
                  } else {
                    fitted_model <- glm(y_train ~ X_train, family = "binomial")
                  }
...
saveRDS(grid_search, file="outputs/grid_search_covid.rds")
```
